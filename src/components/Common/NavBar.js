    ...
    <nav  className="navbar navbar-expand-lg navbar-light fixed-top  shadow-md">
       <div className="container-fluid  container-lg ">
        <a className="navbar-toggler" type="button" data-bs-toggle="collapse" data-bs-target="#navbarSupportedContent" aria-controls="navbarSupportedContent" aria-expanded="false" aria-label="Toggle navigation">
         ...
         <Link className="navbar-brand m-auto  m-md-auto m-sm-auto " to="/">
          <img className="img-fluid" src={require(`../../images/logo/dark_logo.webp`)} alt="" srcSet=""/>
         </Link>
         ...
         <ul className="navbar-nav mb-2  mb-lg-0 d-block d-xl-none d-lg-none ">
          <li className="nav-item user-nav-item  ">
            ...
            </span> <span className="log_in_text" >Log In </span></Link>
          </li>
         </ul>
         ...
         <div className="collapse navbar-collapse " id="navbarSupportedContent">
           <ul className="navbar-nav  m-auto mb-2  mb-lg-0 ">
            {
              nav.map((item)=>
              <li key={item.id} className={`nav-item ${item.id == navId ? 'active' : ''}`}>
                <Link onClick={()=>setNavId(item.id)} className="nav-link" aria-current="page" to={item.to}>{item.name}</Link>
              </li>
           ...
           <ul className="navbar-nav mb-2  mb-lg-0 ">
            ...
            <li className="get_btn_li">
              <a className="get_started_btn" to="">Get Started</a>
            </li>
           </ul>
